...
import { Original_DummyData } from 'utils/dummy-data';
...
const SavedList = (props: ISavedListProps) => {
  // Props
  ...
  // Const
  const { ON_HISTORY_LIST_UPDATE } = SAVED_LIST_EVENTS;
  ...
  const getTransactions = debounce(async (pageNumber = 1) => {
    try {
      ...
      const data: any = Original_DummyData.slice(0, 4);
      if (pageNumber > 1) {
